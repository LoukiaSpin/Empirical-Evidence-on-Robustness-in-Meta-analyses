...
<p>The repository offers the typical structure of separate folders for data, models and R (code/scripts), respectively.<br>
<p>* The _data_ folder includes four RData and text files. The RData files **binary_NMA**, **binary_PMA**, **continuous_NMA**, and **continuous_PMA** refer to the datasets with the analysed pairwise meta-analyses (PMA) and network meta-analyses (NMA) on selected binary and continuous outcomes. The remaining RData files refer to the mean and standard deviation of selected empirically-based prior distributions for the between-trial variance that align with the outcome and intervention-comparison type of each analysis. Finally, the text files contain the posterior summaries from the Bayesian analysis (via the R2jags R-package) under all pre-defined scenarios for the missingness mechanism;
* The _R_ folder includes two analysis scripts (**A.Run empirical analysis_PMA & NMA.R** and **Β.Determine robustness_PMA & NMA.R**). The first script sources the RData files with the datasets and performs the Bayesian analyses (one-stage random-effects PMA and NMA for continuous and binary outcomes with incorporation of the pattern-mixture model). The second script calculates the robustness index and apply our proposed decision framework for robustness of the primary analysis results for each analysis. Furthermore, it produces 1) the heatmap of robustness index for all possible comparisons of the network and 2) the panel of density plots of the summary log OR under all scenarios for the missingness mechanism and the Kullback-Leibler Divergence. The network of Liu et al. (2013) has been used for that purpose. Finally, the remaining R scripts refer to the necessary functions to apply the aforementioned analysis scripts.<br>
...
To prepare the data in the proper format to use [R2jags](https://CRAN.R-project.org/package=R2jags) for the Bayesian random-effects pairwise and network meta-analysis, we have developed the function `data.preparation()` which has the following syntax:
...
* data: The input is a data-frame of a one-trial-per-row format containing arm-level data of each trial. This format is widely used for BUGS models. The columns of the data-frame `data` refer to the following ordered elements for a continuous outcome:
<p>**t** which is an intervention identifier;<br>
**y** which is the observed mean value of the outcome;<br>
**sd** which is the observed standard deviation of the outcome;<br>
**m** which is the number of missing outcome data;<br>
**n** which is the number of participants randomised on the assigned intervention.<br>
<p>For a binary outcome, the columns of the data-frame `data` refer to the following ordered elements:<br>
**t** which is the intervention identifier;<br>
**r** which is the observed number of events of the outcome;<br>
**m** which is the number of missing outcome data;<br>
**n** which is the number of participants randomised on the assigned intervention.<br>
...
We have developed the function `sensitivity.analysis.mod()` to run automatically Bayesian random-effects pairwise and network meta-analysis (with proper accommodation of missing participant outcome data) separately for each pre-defined scenario for the missingness mechanism. The function has the following syntax:
...
* data: See the function `data.preparation()`.
* measure: See the function `data.preparation()`.
...
* heter.prior: A list of three elements with the following order: 1) a character string indicating the distribution with (currently available) values `halfnormal`, `uniform`, `lognormal`, or `logt`; 2) two numbers that refer to the parameters of the selected distribution. This argument informs the `heterogeneity.param.prior()` function that determines the distribution to be used in the `prepare.model()` function (see section _Important details_).
...
<p>This function returns a list with two elements that contain an R2jags output: **EM** which is the effect estimate of all possible comparisons of interventions under each scenario, and **tau** which is the between-trial standard deviation (assumed to be common for all observed comparisons) under each scenario.

#### Important details
<p>This function calls the `prepare.model()` function that contains all models in the BUGS language.
<p> The function `sensitivity.analysis.mod()` is found in the script **A.Run empirical analysis_PMA & NMA.R**.
...
* ES.mat: The input is the **EM** which is the element of the function `sensitivity.analysis.mod()`. 
* threshold: The threshold of robustness. We suggest using 0.17 and 0.28 for a continuous and a binary outcome, respectively.
* primary.scenar: A number to indicate the primary analysis (i.e. the missing at random assumption). We have considered `primary.scenar = 13`, namely, the 13-th scenario.
...
The function `Robustness:Index()` returns a list of two items:
...
3. a vector with the character strings __robust__ or __frail__ that indicate whther the primary analysis results for the corresponding comparison is robust or frail to the alternative missingness scenarios.
...
* RI: A vector with the robustness indexI) calculated for all pairwise comparisons. You need to use, first, the `RobustnessIndex()` function. 
* drug.names: A vector with the names of the interventions compared. The interventions should be in the same order as in the argunemt of the function `sensitivity.analysis.mod()`. This is important particularly in the case of network meta-analysis so that you do not match the robustness index with the wrong comparisons.
* threshold: The threshold of robustness. We suggest using 0.17 and 0.28 for a continuous and a binary outcome, respectively.
...
The function `KLD.plots()` returns a five-by-five panel that illustrates with three density plots: one on the effect measure under the missing at random assumption (the primary analysis), one on the effect measure under an alternative informative scenario, and one on the Kullback-Leibler divergence of these two scenarios. 
