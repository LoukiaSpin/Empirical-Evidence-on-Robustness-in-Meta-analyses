#----------------------------------------------------------------------------------------------------------------------------
#     A function to create the heatmap on Robustness Index for all possible comparisons of interventions in the network
...
HeatMap.AllComparisons.RI <- function(RI, drug.names, threshold){
  ...
  mat.new$conclusions <- ifelse(mat.new$value < threshold, "robust", "frail")
  
  
  ## Create the heatmap for one network of interventions
  ggplot(mat.new, aes(Var2, factor(Var1, level = drug.names[length(drug.names):2]), fill = conclusions)) + 
         geom_tile(colour = "white") + 
         geom_text(aes(Var2, Var1, label = value, fontface = "bold"), size = rel(4.5)) +
         scale_fill_manual(breaks = c("robust", "frail"), values = c("#009E73", "#D55E00")) +
         scale_x_discrete(position = "top") +
         labs(x = "", y = "", fill = "Conclusions") +
         theme_classic() + 
